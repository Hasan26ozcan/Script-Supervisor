...
async def test_phase4_script_runs_in_mock_mode(monkeypatch):
    ...
    for mod in ["app.main", "app.agent_loop", "app.config", "app.prompts", "app.gateway", "app.rubric", "app.schemas"]:
    ...
    assert results["trials"][0]["text_only"]["turns"] == 1 or results["trials"][0]["text_only"]["turns"] == 3
